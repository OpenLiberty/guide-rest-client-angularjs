...
Before your application is bootstrapped, you must pull in two [hotspot=4-5]`AngularJS` libraries and import [hotspot=6]`consume-rest.js`.
...
The application is bootstrapped because the [hotspot=8]`consumeRestApp` application module is attached to the `body` of the template.

Next, the [hotspot=9]`ArtistCtrl` controller is attached to the DOM to create a new child scope. 
...
You can use the [hotspot=10]`ng-repeat` directive to iterate over the contents
