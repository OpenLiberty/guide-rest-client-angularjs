...
The [hotspot=7-13]`controller` controls the flow of data in your application.
...
The application is bootstrapped because the [hotspot=12 file=0]`consumeRestApp` application module is attached to the [hotspot=12-25 file=0]`body` of the template.

Next, the [hotspot=13 file=0]`ArtistCtrl` controller is attached to the DOM to create a new child scope. 

The controller will make the
...
using the [hotspot=15 file=0]`{{ artists }}` expression. 

You can use the [hotspot=14 file=0]`ng-repeat` directive to iterate over the contents
