...
index.html
[source, html, linenums, role='code_column tags=html']
----
...
for the `$resource` service. The application is bootstrapped because the [hotspot=consumeRestApp file=0]`consumeRestApp` application module is attached to the [hotspot=body file=0]`body` of the template.
...
using the [hotspot=artist-info file=0]`{{ artists }}` expression. You can use the [hotspot=ArtistCtrl file=0]`ng-repeat` directive to iterate over the contents
of the [hotspot=ArtistCtrl file=0]`artists` property.
