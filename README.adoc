...
Any local changes to your JavaScript and HTML are picked up automatically, so you do not need to restart
your server.
...
Append the following inside the `html` tag in `start/src/main/webapp/index.html`:
...
Next, add the relevant AngularJS code to bootstrap your application and create a new scope where your
`artists` property will be accessible.

Append the following inside the `html` tag and underneath the `head` tag in `start/src/main/webapp/index.html`:
...
Lets break this down. Attaching the `consumeRestApp` application to the `body` element bootstraps
the application:
...
Attaching, the `ArtistCtrl` controller to the DOM creates a new child scope, from within which the
`artists` property becomes available:

[source, html, role="no_copy", indent=0]
----
include::finish/src/main/webapp/index.html[tags=controller;!repeat]
----
...
[source, html, role="no_copy", indent=0]
----
include::finish/src/main/webapp/index.html[tags=repeat]
----
...
Although you did not build any logic that uses the consumed resources in this guide, you will likely
need to when you develop your own applications. In that case, testing will become a crucial part of
your development lifecycle. If you need to write test cases, follow the official unit testing and
end-to-end testing documentation on the AngularJS website: https://docs.angularjs.org/guide/unit-testing.
