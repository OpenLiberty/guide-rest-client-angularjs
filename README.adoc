...
The [hotspot=1]`ngResource` module is registered as it is appended after [hotspot=1]`consumeRestApp`. 
...
Next, the [hotspot=3-5]`Artists` AngularJS service is defined by using the Factory recipe. 
...
You will create the starting point of your application under `start/src/main/webapp/index.html`.
This file will contain all elements and attributes specific to AngularJS.

[role="code_command hotspot=1-26", subs="quotes"]
----
...
Before your application is bootstrapped, you must pull in two [hotspot=4-9]`AngularJS` libraries and import [hotspot=10]`consume-rest.js`.
...
The application is bootstrapped because the [hotspot=12]`consumeRestApp` application module is attached to the `body` of the template.

Next, the [hotspot=13]`ArtistCtrl` controller is attached to the DOM to create a new child scope. 
...
You can use the [hotspot=14]`ng-repeat` directive to iterate over the contents
