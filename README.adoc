...
Begin by registering your application module. Every application must contain at least one module, the application
module, that will be bootstrapped in order to launch the application.
...
You will need some way of communicating with RESTful web services in order to retrieve their resources.
In the case of this guide, you will need to communicate with the artist service to retrieve the artist JSON.
While there exists a variety of ways about doing this, you can use the fairly straightforward AngularJS
`$resource` service.
...
Before you bootstrap the application, first pull in two AngularJS libraries and import `consume-rest.js`:
...
**Implement the following three snippets in order, building them on top of one another.**

Bootstrap your application with the `consumeRestApp` application module:
...
Attach the `ArtistCtrl` controller to the DOM:
...
AngularJS website: https://docs.angularjs.org/guide/unit-testing[Unit Testing].
